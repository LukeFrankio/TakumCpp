...
TEST(SpecReference, RoundTrip32) {
    ...
    long double back = takum::internal::ref::high_precision_decode<32>(static_cast<uint64_t>(a.storage));
    // Reference codec is crude; assert round-trip within a reasonable factor using high precision for decoded value (proxy for full τ)
    ASSERT_NEAR(back, 3.141592653589793L, 1e-2L);
    // For exactness, compare internal tuple (S,c,m) or ℓ via ref if available
}
...
TEST(SpecReference, NaR32) {
    ...
    long double nar_dec = takum::internal::ref::high_precision_decode<32>(static_cast<uint64_t>(a.storage));
...
TEST(SpecReference, RoundTrip64) {
    ...
    long double back = takum::internal::ref::high_precision_decode<64>(static_cast<uint64_t>(a.storage));
